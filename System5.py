...
def display_final_results(tape_exec, memory_bank, target_dt, target_date_str, micro_tf=None, macro_tfs=None):
    ...
    # ---- Per-underlying summary (counts by underlying symbol, both baskets,
    # broken out by CE/PE where applicable - e.g. index/stock options) ----
    active_counts, closed_counts = defaultdict(int), defaultdict(int)
    active_ce, active_pe, closed_ce, closed_pe = defaultdict(int), defaultdict(int), defaultdict(int), defaultdict(int)
    for st in active_runners:
        u = _extract_underlying(st["sym"]); active_counts[u] += 1
        opt_type = _extract_option_type(st["sym"])
        if opt_type == "CE": active_ce[u] += 1
        elif opt_type == "PE": active_pe[u] += 1
    for st in closed_trades:
        u = _extract_underlying(st["sym"]); closed_counts[u] += 1
        opt_type = _extract_option_type(st["sym"])
        if opt_type == "CE": closed_ce[u] += 1
        elif opt_type == "PE": closed_pe[u] += 1

    has_options = bool(active_ce or active_pe or closed_ce or closed_pe)
    all_underlyings = sorted(
        set(active_counts) | set(closed_counts),
        key=lambda u: (-active_counts.get(u, 0), -closed_counts.get(u, 0), u)
    )

    print(f"{COLOR_CYAN}------------------------------------------------------------------------------------------------{COLOR_RESET}")
    print(f"{COLOR_BOLD}📊 SUMMARY — Trade Count by Underlying [{micro_tf}]{COLOR_RESET}")
    if all_underlyings:
        if has_options:
            print(f"  {'Underlying':<14}{'B1 Total':<10}{'B1 CE':<8}{'B1 PE':<8}{'B2 Total':<10}{'B2 CE':<8}{'B2 PE':<8}")
            for u in all_underlyings:
                print(f"  {u:<14}{active_counts.get(u,0):<10}{active_ce.get(u,0):<8}{active_pe.get(u,0):<8}"
                      f"{closed_counts.get(u,0):<10}{closed_ce.get(u,0):<8}{closed_pe.get(u,0):<8}")
            print(f"  {'-'*66}")
            print(f"  {'TOTAL':<14}{sum(active_counts.values()):<10}{sum(active_ce.values()):<8}{sum(active_pe.values()):<8}"
                  f"{sum(closed_counts.values()):<10}{sum(closed_ce.values()):<8}{sum(closed_pe.values()):<8}")
        else:
            print(f"  {'Underlying':<16}{'Basket1 (Active)':<20}{'Basket2 (Closed)':<20}")
            for u in all_underlyings:
                print(f"  {u:<16}{active_counts.get(u, 0):<20}{closed_counts.get(u, 0):<20}")
            print(f"  {'-'*56}")
            print(f"  {'TOTAL':<16}{sum(active_counts.values()):<20}{sum(closed_counts.values()):<20}")
    else:
