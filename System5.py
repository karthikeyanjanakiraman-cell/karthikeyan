...
def get_index_universe_and_options(target_indices):
    ...
    if nse_indices:
        try:
            res_fo = requests.get("https://public.fyers.in/sym_details/NSE_FO.csv", headers=headers, timeout=15)
            opt_inst.extend(_parse_fo_csv_for_indices(res_fo.text, nse_indices))
        except Exception as e:
            print(f"  ├─ {COLOR_YELLOW}⚠️ Could not fetch NSE_FO.csv: {e}{COLOR_RESET}")

    if bse_indices:
        try:
            res_fo = requests.get("https://public.fyers.in/sym_details/BSE_FO.csv", headers=headers, timeout=15)
            opt_inst.extend(_parse_fo_csv_for_indices(res_fo.text, bse_indices))
        except Exception as e:
            print(f"  ├─ {COLOR_YELLOW}⚠️ Could not fetch BSE_FO.csv: {e}{COLOR_RESET}")
...
def evaluate_single_timeframe_gates(df_base, tf_str, tier_type="MACRO"):
    ...
    confirm_bricks = MACRO_RENKO_CONFIRM_BRICKS if tier_type == "MACRO" else MICRO_RENKO_CONFIRM_BRICKS
    df_tf = construct_45deg_renko_matrix(df_tf, tf_str, confirm_bricks)
    df_tf = construct_volume_delta_renko_matrix(df_tf, tf_str, confirm_bricks)
    df_tf = construct_renko_velocity_engine(df_tf, tf_str)
    df_tf = construct_bb_meta_pillars(df_tf, tf_str)
    df_tf = apply_dual_tier_scorecard(df_tf, tf_str, tier_type)
...
def prepare_unified_execution_tape(rolling_master_df, micro_timeframes, macro_timeframes, strategy_mode="BOTH"):
    ...
    if exec_tf != "1min":
        ...
        df_micro = rolling_master_df.groupby(["Symbol", pd.Grouper(key="Datetime", freq=exec_tf, closed="left", label="left", origin=origin)]).agg(
            {"Open": "first", "High": "max", "Low": "min", "Close": "last", "Volume": "sum", "Net_Delta_1m": "sum"}
        ).reset_index().dropna(subset=["Close"]).sort_values(["Symbol", "Datetime"])
    ...
    for tf in micro_timeframes[1:]:
        ...
        b_col, br_col = f"Armed_Bull_{tf}", f"Armed_Bear_{tf}"
        micro_bull_gates.append(b_col); micro_bear_gates.append(br_col)
        env_df["Datetime"] = env_df["Datetime"].astype("datetime64[ns]")
        df_micro = pd.merge_asof(df_micro.sort_values("Datetime"), env_df.sort_values("Datetime"), on="Datetime", by="Symbol", direction="backward")
        df_micro[b_col] = df_micro[b_col].fillna(False)
        df_micro[br_col] = df_micro[br_col].fillna(False)

    n_micro_tfs = len(micro_bull_gates)
    micro_bull_agree = df_micro[micro_bull_gates].sum(axis=1)
    micro_bear_agree = df_micro[micro_bear_gates].sum(axis=1)
    if MICRO_CONFIRMATION_MODE == "ALL":
...
def _run_dual_layer_trade_management(tape_exec, micro_timeframes, macro_timeframes, cutoff_time_obj):
    ...
    micro_p_renkos = {tf: tape_exec.set_index(["Datetime", "Symbol"])[f"Renko_Count_{tf}"].to_dict() for tf in micro_timeframes}
    micro_v_renkos = {tf: tape_exec.set_index(["Datetime", "Symbol"])[f"Vol_Renko_Count_{tf}"].to_dict() for tf in micro_timeframes}
                ...
                if (ltp := closes_dict.get((t_dt, sym))) is not None:
                    ...
                    if not exit_reason:
                        ...
                        for tf in micro_timeframes:
                            entry_mi_p = st["entry_micro_p"].get(tf)
                            entry_mi_v = st["entry_micro_v"].get(tf)
                            if entry_mi_p is None: continue
                            mi_p = micro_p_renkos[tf].get((t_dt, sym), entry_mi_p)
                            mi_v = micro_v_renkos[tf].get((t_dt, sym), entry_mi_v)
                            if st["dir"] == 1:
                                tf_reversed = (mi_p <= (entry_mi_p - MICRO_EXIT_PRICE_BRICKS)) or (mi_v <= (entry_mi_v - MICRO_EXIT_VOL_BRICKS))
                            else:
                                tf_reversed = (mi_p >= (entry_mi_p + MICRO_EXIT_PRICE_BRICKS)) or (mi_v >= (entry_mi_v + MICRO_EXIT_VOL_BRICKS))
                            if tf_reversed: reversed_micro_tfs.append(tf)
                        required_micro_exit = _vote_required(MICRO_EXIT_CONFIRMATION_MODE, len(micro_timeframes))
                    ...
                    if not exit_reason:
                        ...
                        for tf in triggering_tfs:
                            ...
                            if st["dir"] == 1:
                                tf_reversed = (mac_p <= (entry_mac_p - MACRO_EXIT_PRICE_BRICKS)) or (mac_v <= (entry_mac_v - MACRO_EXIT_VOL_BRICKS))
                            else:
                                tf_reversed = (mac_p >= (entry_mac_p + MACRO_EXIT_PRICE_BRICKS)) or (mac_v >= (entry_mac_v + MACRO_EXIT_VOL_BRICKS))
                            if tf_reversed: reversed_tfs.append(tf)

                        n_triggering = len(triggering_tfs)
                        if n_triggering > 0:
                            required_exit = _vote_required(MACRO_EXIT_CONFIRMATION_MODE, n_triggering)
                            if len(reversed_tfs) >= required_exit:
                                exit_reason = f"Macro Reversal [{MACRO_EXIT_CONFIRMATION_MODE}] ({', '.join(reversed_tfs)})"
        ...
        if t_dt in anomalies_by_time.groups and t_dt.time() < cutoff_time_obj:
            for _, row in anomalies_by_time.get_group(t_dt).iterrows():
                ...
                memory_bank.setdefault(sym, []).append({
                    ...
                    "entry_renko_count": row.get(f"Renko_Count_{exec_tf}", 0), "entry_vol_renko_count": row.get(f"Vol_Renko_Count_{exec_tf}", 0),
                    "current_renko_count": row.get(f"Renko_Count_{exec_tf}", 0), "last_brick_formed_dt": t_dt,
                    ...
                    "entry_micro_p": {tf: micro_p_renkos[tf].get((t_dt, sym), 0) for tf in micro_timeframes},
                    "entry_micro_v": {tf: micro_v_renkos[tf].get((t_dt, sym), 0) for tf in micro_timeframes},
                    "micro_score": row.get(f"Score_Bull_{exec_tf}" if direction == 1 else f"Score_Bear_{exec_tf}", 0)
                })
...
def _parse_option_symbol(sym):
    ...
    if m.group("mon3"):
        expiry_label = f"{m.group('mon3')}'{m.group('yy')} Monthly"
    else:
        mon_name = _WEEKLY_MONTH_CHAR.get(m.group("mchar"), m.group("mchar"))
...
def display_final_results(tape_exec, memory_bank, target_dt, target_date_str, micro_tfs=None, macro_tfs=None):
    ...
    micro_display_str = " | ".join(micro_tfs) + (f" ({MICRO_CONFIRMATION_MODE})" if len(micro_tfs) > 1 else "")
    macro_display_str = " | ".join(macro_tfs) + (f" ({MACRO_CONFIRMATION_MODE})" if len(macro_tfs) > 1 else "")
    print(f"\n{COLOR_CYAN}================================================================================================{COLOR_RESET}")
    print(f"{COLOR_BOLD}9-PILLAR ENGINE [Micro: {micro_display_str} ⚡ Macro: {macro_display_str}] — RESULTS [{TRADING_MODE}]{COLOR_RESET}")
    ...
    print(f"{COLOR_CYAN}------------------------------------------------------------------------------------------------{COLOR_RESET}")
    print(f"{COLOR_BOLD}📊 SUMMARY — Trade Count by Strike [{micro_display_str}]{COLOR_RESET}")
    if all_rows:
...
def scan_institutional_tape(target_date_str, entry_cutoff_time_str=ENTRY_CUTOFF_TIME):
    ...
    if TRADING_MODE == "CASH_EQUITY":
        ...
        print(f"{COLOR_RED}❌ Unrecognized TRADING_MODE '{TRADING_MODE}'. Must be one of: CASH_EQUITY, INDEX_OPTIONS, STOCK_FNO.{COLOR_RESET}")
        return
    ...
    if not tape_exec.empty:
        memory_bank = _run_dual_layer_trade_management(tape_exec, MICRO_TIMEFRAMES, MACRO_TIMEFRAMES, cutoff_time_obj)
        display_final_results(tape_exec, memory_bank, pd.to_datetime(target_date_str), target_date_str, micro_tfs=MICRO_TIMEFRAMES, macro_tfs=MACRO_TIMEFRAMES)
