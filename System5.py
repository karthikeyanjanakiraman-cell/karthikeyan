### Chunk 1: Configuration & Imports
###Paste this at the very top. It sets up your zero-lag parameters and execution modes.
...
print("🔖 SYSTEM ENGINE BUILD: LIVE-TRAVERSAL-UPDATE (Rolling MTF)")

# --- Engine Constants & Colors ---
...
MACRO_TIMEFRAMES = ["420min"] # Full daily rolling window
...
# --- Tier 3 & 4: Trade Exits and Options ---
...
def validate_fyers_token():
    if not os.environ.get("FYERS_CLIENT_ID") or not os.environ.get("FYERS_ACCESS_TOKEN"):
        print(f"❌ {COLOR_RED}Error: FYERS credentials missing in environment variables.{COLOR_RESET}")
        return False
    try:
        res = requests.get("[https://api-t1.fyers.in/api/v3/profile](https://api-t1.fyers.in/api/v3/profile)", headers=get_fyers_auth_headers(), timeout=10)
        body = res.json() if res.status_code == 200 else {}
        if res.status_code == 200 and body.get("s") == "ok":
            print(f"{COLOR_GREEN}✅ Token validated (Account: {body.get('data', {}).get('name', 'Unknown')}){COLOR_RESET}")
            return True
        return False
...
def get_cash_equity_universe():
    spot_inst = []
    try:
        res = requests.get("[https://public.fyers.in/sym_details/NSE_CM.csv](https://public.fyers.in/sym_details/NSE_CM.csv)", headers={"User-Agent": "Mozilla"}, timeout=15)
...
def get_fno_universe_and_options():
    ...
    try:
        ...
        res_cm = requests.get("[https://public.fyers.in/sym_details/NSE_CM.csv](https://public.fyers.in/sym_details/NSE_CM.csv)", headers=headers, timeout=15)
        spot_map = {}
        ...
        res_fo = requests.get("[https://public.fyers.in/sym_details/NSE_FO.csv](https://public.fyers.in/sym_details/NSE_FO.csv)", headers=headers, timeout=15)
        valid_und = set()
...
def fetch_fyers_candles(key, start_dt, end_dt, resolution="1"):
    ...
    for attempt in range(3):
        try:
            time.sleep(0.15)
            url = f"[https://api-t1.fyers.in/data/history?symbol=](https://api-t1.fyers.in/data/history?symbol=){urllib.parse.quote(key, safe=':')}&resolution={resolution}&date_format=1&range_from={start_dt}&range_to={end_dt}"
...
def calculate_core_technicals(frame_tf):
    frame_tf["H-L"] = frame_tf["High"] - frame_tf["Low"]
    frame_tf["H-PC"] = (frame_tf["High"] - frame_tf.groupby("Symbol")["Close"].shift(1)).abs()
    frame_tf["L-PC"] = (frame_tf["Low"] - frame_tf.groupby("Symbol")["Close"].shift(1)).abs()
    frame_tf["TR"] = frame_tf[["H-L", "H-PC", "L-PC"]].max(axis=1)
    frame_tf["ATR"] = frame_tf.groupby("Symbol")["TR"].transform(lambda x: x.rolling(ATR_PERIOD, min_periods=1).mean()).fillna(frame_tf["Close"] * RENKO_DEFAULT_PCT)

    delta = frame_tf.groupby("Symbol")["Close"].diff()
    gain, loss = delta.where(delta > 0, 0), -delta.where(delta < 0, 0)
    avg_gain = gain.groupby(frame_tf["Symbol"]).transform(lambda x: x.rolling(RSI_PERIOD, min_periods=1).mean())
    avg_loss = loss.groupby(frame_tf["Symbol"]).transform(lambda x: x.rolling(RSI_PERIOD, min_periods=1).mean())
    
    frame_tf["RSI"] = 100 - (100 / (1 + (avg_gain / (avg_loss + 1e-8))))
    
    # TRUE BB-RSI
    frame_tf["RSI_SMA"] = frame_tf.groupby("Symbol")["RSI"].transform(lambda x: x.rolling(BB_SMA_PERIOD, min_periods=1).mean())
    frame_tf["RSI_STD"] = frame_tf.groupby("Symbol")["RSI"].transform(lambda x: x.rolling(BB_SMA_PERIOD, min_periods=1).std()).fillna(0)
    ...
    h_diff = frame_tf["High"] - frame_tf.groupby("Symbol")["High"].shift(1)
    l_diff = frame_tf.groupby("Symbol")["Low"].shift(1) - frame_tf["Low"]
    frame_tf["+DM"] = np.where((h_diff > l_diff) & (h_diff > 0), h_diff, 0)
    frame_tf["-DM"] = np.where((l_diff > h_diff) & (l_diff > 0), l_diff, 0)
    frame_tf["+DI"] = 100 * (frame_tf.groupby("Symbol")["+DM"].transform(lambda x: x.rolling(ADX_PERIOD, min_periods=1).mean()) / (frame_tf["ATR"] + 1e-8))
    frame_tf["-DI"] = 100 * (frame_tf.groupby("Symbol")["-DM"].transform(lambda x: x.rolling(ADX_PERIOD, min_periods=1).mean()) / (frame_tf["ATR"] + 1e-8))
    frame_tf["DX"] = 100 * abs(frame_tf["+DI"] - frame_tf["-DI"]) / (frame_tf["+DI"] + frame_tf["-DI"] + 1e-8)
    frame_tf["ADX"] = frame_tf.groupby("Symbol")["DX"].transform(lambda x: x.rolling(ADX_PERIOD, min_periods=1).mean())

    frame_tf["EMA_8"] = frame_tf.groupby("Symbol")["Close"].transform(lambda x: x.rolling(8, min_periods=1).mean())
    frame_tf["EMA_21"] = frame_tf.groupby("Symbol")["Close"].transform(lambda x: x.rolling(21, min_periods=1).mean())
    frame_tf["EMA_Spread"] = abs(frame_tf["EMA_8"] - frame_tf["EMA_21"])
    spread_limit = frame_tf.groupby("Symbol")["EMA_Spread"].transform(lambda x: x.rolling(20, min_periods=1).mean()) * 0.20
...
def construct_renko_velocity_engine(df, tf_name):
    ...
    active = df[f"Mins_Since_{tf_name}"] <= (RENKO_VELOCITY_MAX_BARS * _parse_tf_to_minutes(tf_name))
...
def construct_bb_meta_pillars(df, tf_name):
    # Corrected Expansion Bands
    atr_m = df.groupby("Symbol")["ATR"].transform(lambda x: x.rolling(BB_SMA_PERIOD, min_periods=1).mean())
    atr_s = df.groupby("Symbol")["ATR"].transform(lambda x: x.rolling(BB_SMA_PERIOD, min_periods=1).std()).fillna(0)
...
# --- Rolling MTF Evaluation Engine ---
def evaluate_single_timeframe_gates(df_base, tf_str, tier_type="MACRO"):
    tf_mins = _parse_tf_to_minutes(tf_str)
    frame_tf = df_base.copy().sort_values(["Symbol", "Datetime"])
    
    # 🔴 Rolling Traversal Logic applied here 
    if tf_mins > 1:
        frame_tf["Open"] = frame_tf.groupby("Symbol")["Open"].transform(lambda x: x.shift(tf_mins - 1).bfill())
        frame_tf["High"] = frame_tf.groupby("Symbol")["High"].transform(lambda x: x.rolling(window=tf_mins, min_periods=1).max())
        frame_tf["Low"] = frame_tf.groupby("Symbol")["Low"].transform(lambda x: x.rolling(window=tf_mins, min_periods=1).min())
        frame_tf["Volume"] = frame_tf.groupby("Symbol")["Volume"].transform(lambda x: x.rolling(window=tf_mins, min_periods=1).sum())
        frame_tf["Timeframe_Net_Delta"] = frame_tf.groupby("Symbol")["Net_Delta_1m"].transform(lambda x: x.rolling(window=tf_mins, min_periods=1).sum())
    else:
        frame_tf["Timeframe_Net_Delta"] = frame_tf["Net_Delta_1m"]
        
    frame_tf["Net_Delta_Pct"] = (frame_tf["Timeframe_Net_Delta"] / (frame_tf["Volume"] + 1e-9)) * 100
    frame_tf = calculate_core_technicals(frame_tf)
    
    c_bricks = MACRO_RENKO_CONFIRM_BRICKS if tier_type == "MACRO" else MICRO_RENKO_CONFIRM_BRICKS
    frame_tf = construct_45deg_renko_matrix(frame_tf, tf_str, c_bricks)
    frame_tf = construct_volume_delta_renko_matrix(frame_tf, tf_str, c_bricks)
    frame_tf = construct_renko_velocity_engine(frame_tf, tf_str)
    frame_tf = construct_bb_meta_pillars(frame_tf, tf_str)
    frame_tf = apply_dual_tier_scorecard(frame_tf, tf_str, tier_type)
    
    cols = ["Symbol", "Datetime", f"Armed_Bull_{tf_str}", f"Armed_Bear_{tf_str}", 
            f"Score_Bull_{tf_str}", f"Score_Bear_{tf_str}", f"Renko_Count_{tf_str}", 
            f"Vol_Renko_Count_{tf_str}", f"Mins_Since_{tf_str}"]
    return frame_tf[cols].copy().sort_values("Datetime").reset_index(drop=True)

def prepare_unified_execution_tape(master_df, micro_tfs, macro_tfs, strat_mode="BOTH"):
    exec_tf = micro_tfs[0]  
    tf_mins = _parse_tf_to_minutes(exec_tf)
    
    df_micro = master_df.copy().sort_values(["Symbol", "Datetime"])
    
    if tf_mins > 1:
        df_micro["Open"] = df_micro.groupby("Symbol")["Open"].transform(lambda x: x.shift(tf_mins - 1).bfill())
        df_micro["High"] = df_micro.groupby("Symbol")["High"].transform(lambda x: x.rolling(window=tf_mins, min_periods=1).max())
        df_micro["Low"] = df_micro.groupby("Symbol")["Low"].transform(lambda x: x.rolling(window=tf_mins, min_periods=1).min())
        df_micro["Volume"] = df_micro.groupby("Symbol")["Volume"].transform(lambda x: x.rolling(window=tf_mins, min_periods=1).sum())
        df_micro["Timeframe_Net_Delta"] = df_micro.groupby("Symbol")["Net_Delta_1m"].transform(lambda x: x.rolling(window=tf_mins, min_periods=1).sum())
    else:
    ...
    df_micro = construct_45deg_renko_matrix(df_micro, exec_tf, MICRO_RENKO_CONFIRM_BRICKS)
    df_micro = construct_volume_delta_renko_matrix(df_micro, exec_tf, MICRO_RENKO_CONFIRM_BRICKS)
    df_micro = construct_renko_velocity_engine(df_micro, exec_tf)
    df_micro = construct_bb_meta_pillars(df_micro, exec_tf)
    ...
    for tf in macro_tfs:
        env_df = evaluate_single_timeframe_gates(master_df, tf, "MACRO")
    ...
    mi_bull_gates, mi_bear_gates = [f"Armed_Bull_{exec_tf}"], [f"Armed_Bear_{exec_tf}"]
    for tf in micro_tfs[1:]:
        env_df = evaluate_single_timeframe_gates(master_df, tf, "MICRO")
        b_col, br_col = f"Armed_Bull_{tf}", f"Armed_Bear_{tf}"
...
def _run_dual_layer_trade_management(tape, micro_tfs, macro_tfs, cutoff_time):
    exec_tf = micro_tfs[0]
    ...
    mi_p_r = {tf: tape.set_index(["Datetime", "Symbol"])[f"Renko_Count_{tf}"].to_dict() for tf in micro_tfs}
    mi_v_r = {tf: tape.set_index(["Datetime", "Symbol"])[f"Vol_Renko_Count_{tf}"].to_dict() for tf in micro_tfs}
    ma_p_r = {tf: tape.set_index(["Datetime", "Symbol"])[f"Renko_Count_{tf}"].to_dict() for tf in macro_tfs}
    ma_v_r = {tf: tape.set_index(["Datetime", "Symbol"])[f"Vol_Renko_Count_{tf}"].to_dict() for tf in macro_tfs}

    bank, last_px, last_dir, daily_cnt = {}, {}, {}, defaultdict(int)
    max_stall = RENKO_VELOCITY_MAX_BARS * _parse_tf_to_minutes(exec_tf)
    ...
    for t in np.sort(tape["Datetime"].unique()):
        ...
        for sym, eps in bank.items():
            if eps and eps[-1]["state"] == "ACTIVE":
                ...
                if (ltp := closes.get((t_dt, sym))) is not None:
                    ...
                    cur_p = mi_p_r[exec_tf].get((t_dt, sym), 0)
                    ...
                    if not reason:
                        rev_mi = []
                        for tf in micro_tfs:
                            ent_p, ent_v = st["ent_mi_p"].get(tf), st["ent_mi_v"].get(tf)
                            if ent_p is None: continue
                            cur_mi_p, cur_mi_v = mi_p_r[tf].get((t_dt, sym), ent_p), mi_v_r[tf].get((t_dt, sym), ent_v)
                            if st["dir"] == 1:
                                if cur_mi_p <= (ent_p - MICRO_EXIT_PRICE_BRICKS) or cur_mi_v <= (ent_v - MICRO_EXIT_VOL_BRICKS): rev_mi.append(tf)
                            else:
                                if cur_mi_p >= (ent_p + MICRO_EXIT_PRICE_BRICKS) or cur_mi_v >= (ent_v + MICRO_EXIT_VOL_BRICKS): rev_mi.append(tf)
                        if len(rev_mi) >= _req_votes(MICRO_EXIT_CONFIRMATION_MODE, len(micro_tfs)):
                            reason = f"Micro Reversal ({','.join(rev_mi)})"

                    if not reason:
                        trig_ma = st.get("trig_ma", [])
                        rev_ma = []
                        for tf in trig_ma:
                            ent_p, ent_v = st["ent_ma_p"].get(tf), st["ent_ma_v"].get(tf)
                            if ent_p is None: continue
                            cur_ma_p, cur_ma_v = ma_p_r[tf].get((t_dt, sym), ent_p), ma_v_r[tf].get((t_dt, sym), ent_v)
                            if st["dir"] == 1:
                                if cur_ma_p <= (ent_p - MACRO_EXIT_PRICE_BRICKS) or cur_ma_v <= (ent_v - MACRO_EXIT_VOL_BRICKS): rev_ma.append(tf)
                            else:
                                if cur_ma_p >= (ent_p + MACRO_EXIT_PRICE_BRICKS) or cur_ma_v >= (ent_v + MACRO_EXIT_VOL_BRICKS): rev_ma.append(tf)
                        if trig_ma and len(rev_ma) >= _req_votes(MACRO_EXIT_CONFIRMATION_MODE, len(trig_ma)):
                            reason = f"Macro Reversal ({','.join(rev_ma)})"
        ...
        if t_dt in anom_by_time.groups and t_dt.time() < cutoff_time:
            for _, row in anom_by_time.get_group(t_dt).iterrows():
                ...
                trig_ma = [tf for tf in macro_tfs if row.get(f"Armed_Bull_{tf}" if d==1 else f"Armed_Bear_{tf}", False)]
                bank.setdefault(sym, []).append({
                    "state": "ACTIVE", "origin": row["Close"], "date": t_dt.strftime("%Y-%m-%d"), "time": t_dt.strftime("%H:%M"), "dir": d,
                    "curr_r": row.get(f"Renko_Count_{exec_tf}", 0), "last_brick_dt": t_dt,
                    "exit_time": None, "exit_price": None, "exit_reason": None,
                    "trig_ma": trig_ma,
                    "ent_ma_p": {tf: ma_p_r[tf].get((t_dt, sym), 0) for tf in trig_ma},
                    "ent_ma_v": {tf: ma_v_r[tf].get((t_dt, sym), 0) for tf in trig_ma},
                    "ent_mi_p": {tf: mi_p_r[tf].get((t_dt, sym), 0) for tf in micro_tfs},
                    "ent_mi_v": {tf: mi_v_r[tf].get((t_dt, sym), 0) for tf in micro_tfs},
                })
                daily_cnt[sym] += 1
...
def display_final_results(tape, bank, target_dt, target_str):
    ...
    if act:
        print(f"{COLOR_BOLD}🟢 ACTIVE RUNNERS{COLOR_RESET}")
        for st in act:
            if st["sym"] not in ltp_dict: continue
            pnl = (((ltp_dict[st["sym"]] - st["origin"]) / st["origin"]) * 100) if st["dir"] == 1 else (((st["origin"] - ltp_dict[st["sym"]]) / st["origin"]) * 100)
            c = COLOR_GREEN if pnl >= 0 else COLOR_RED
            print(f"  {c}⚡ {st['sym']:<26} P&L: {pnl:+.2f}% | Entry: ₹{st['origin']:.2f} | LTP: ₹{ltp_dict[st['sym']]:.2f}{COLOR_RESET}")

    if clo:
        print(f"\n{COLOR_BOLD}🛑 CLOSED TRADES{COLOR_RESET}")
        for st in clo:
...
# --- Live WebSocket & Pipeline Router ---
        ...
        while True:
            ...
            if now.minute != curr_min:
                ...
                if new_rows:
                    ...
                    tape = prepare_unified_execution_tape(self.hist_df, MICRO_TIMEFRAMES, MACRO_TIMEFRAMES, strat)
                    bank = _run_dual_layer_trade_management(tape, MICRO_TIMEFRAMES, MACRO_TIMEFRAMES, self.cutoff_obj)
...
def run_production_sweep():
    ...
    print(f"\n📡 Initiating Pipeline [{TRADING_MODE}] for {date_str}...")
    ...
    if master_df.empty: return print(f"{COLOR_YELLOW}⚠️ No execution data built. Exiting.{COLOR_RESET}")
    ...
    if DATA_FEED_MODE == "WEBSOCKET" and WS_AVAILABLE:
        ...
        strat = GLOBAL_MACRO_STRATEGY_2D if TRADING_MODE == "CASH_EQUITY" else OPTIONS_STRATEGY_2D
        tape = prepare_unified_execution_tape(master_df, MICRO_TIMEFRAMES, MACRO_TIMEFRAMES, strat)
        if not tape.empty:
            bank = _run_dual_layer_trade_management(tape, MICRO_TIMEFRAMES, MACRO_TIMEFRAMES, pd.to_datetime(f"{date_str} {ENTRY_CUTOFF_TIME}").time())
