...
def fetch_option_chain_equity(symbol: str, expiry: str) -> pd.DataFrame:
    if fyers is None:
        return pd.DataFrame()
    try:
        payload = {'symbol': format_equity_symbol(symbol), 'strikecount': '50', 'timestamp': expiry}
        ...
        if 'strike_price' not in df.columns:
            for alt in ['strikeprice', 'strike']:
                if alt in cols:
                    df['strike_price'] = pd.to_numeric(df[cols[alt]], errors='coerce')
        else:
            df['strike_price'] = pd.to_numeric(df['strike_price'], errors='coerce')
        if 'option_type' not in df.columns:
            for alt in ['optiontype', 'type']:
                if alt in cols:
                    df['option_type'] = df[cols[alt]].astype(str)
        if 'symbol' not in df.columns:
            for alt in ['fyerssymbol', 'tradingsymbol']:
                if alt in cols:
                    df['symbol'] = df[cols[alt]].astype(str)
        if 'ltp' not in df.columns and 'last_price' in cols:
            df['ltp'] = pd.to_numeric(df[cols['last_price']], errors='coerce')
        if 'oi' not in df.columns and 'open_interest' in cols:
            df['oi'] = pd.to_numeric(df[cols['open_interest']], errors='coerce')
        if 'prev_oi' not in df.columns:
            for alt in ['prevoi', 'previous_oi', 'previousoi']:
                if alt in cols:
                    df['prev_oi'] = pd.to_numeric(df[cols[alt]], errors='coerce')
        return df
    except Exception as e:
