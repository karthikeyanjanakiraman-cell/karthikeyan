...
def long_short_label(raw_label: str, side: str) -> str:
    if side == "long": return {"Buy": "LONG", "Buy+": "LONG+", "Buy++": "LONG++"}.get(raw_label, "Neutral")
    if side == "short": return {"Sell": "SHORT", "Sell+": "SHORT+", "Sell++": "SHORT++"}.get(raw_label, "Neutral")
    return raw_label
...
def apply_display_labels(df: pd.DataFrame, side: str) -> pd.DataFrame:
    ...
    for col in ["5m_Signal", "15m_Signal", "30m_Signal", "60m_Signal", "Bull_Signal", "Bear_Signal", "Overall_Signal"]:
        if col in out.columns: out[col] = out[col].apply(lambda x: long_short_label(str(x), side))
    return out
...
def intraday_window_score(df: pd.DataFrame, window_minutes: int = 5) -> float:
    ...
    if cur.empty or len(cur) < 2: return np.nan
...
def compare_window_signal(current_score: float, previous_score: float) -> Tuple[float, str]:
    if pd.isna(current_score) or pd.isna(previous_score): return np.nan, "Neutral"
...
def build_iteration_history(df: pd.DataFrame, window_minutes: int = 5, iterations: int = ITERATIONS_TO_KEEP) -> pd.DataFrame:
    ...
    for i in range(len(d)):
        ...
        if end_ts < start_anchor: continue
        start_ts = end_ts - timedelta(minutes=window_minutes)
        cur = d[(d["timestamp"] >= start_ts) & (d["timestamp"] <= end_ts)]
        if cur.empty or len(cur) < 2: continue
        first_close = safe_float(cur["close"].iloc[0])
        last_close = safe_float(cur["close"].iloc[-1])
...
def choose_top_candidates(summary_df: pd.DataFrame, top_n: int = 10) -> Tuple[pd.DataFrame, pd.DataFrame]:
    if summary_df is None or summary_df.empty: return pd.DataFrame(), pd.DataFrame()
...
def scan_single_option(option_symbol: str, option_type: str, strike: float, underlying: str) -> Optional[Dict]:
    ...
    summary.update({"Underlying": underlying, "Option Type": option_type, "Option Symbol": option_symbol, "Strike": strike, "OBV": compute_today_obv(intra_df)})
    return summary
    ...
    for underlying in candidates_df["Symbol"].dropna().astype(str):
        ...
        for _, row in pair_df.iterrows():
            ...
            for opt_type in ["CE", "PE"]:
                ...
                if safe_float(scanned.get("LTP"), 0.0) < MIN_OPTION_LTP: continue
                rows.append(scanned)
...
def _cell_bg(col: str, value: str) -> str:
    ...
    if col == "% Change":
        ...
        return '#2e7d32' if num > 0 else '#c62828'
...
def _text_color(bg: str) -> str: return '#ffffff'

def colored_table_html(df: pd.DataFrame, columns: List[str], title: str) -> str:
    html = [f"<div style='margin:10px 0 4px 0;'><b>{title}</b></div>"]
    if df is None or df.empty: return "".join(html) + "<div>No candidates.</div>"
    view = df[[c for c in columns if c in df.columns]].copy()
    html.append("<table style='border-collapse:collapse; font-size:10px;'>")
    html.append("<tr>" + "".join([f"<th style='background:#2f3b59; color:#fff; padding:5px;'>{c}</th>" for c in view.columns]) + "</tr>")
    for _, row in view.iterrows():
        html.append("<tr>")
        for c in view.columns:
            cell_val = format_cell(c, row[c])
            bg = _cell_bg(c, cell_val)
            html.append(f"<td style='background:{bg}; color:#fff; text-align:center; padding:4px;'>{cell_val}</td>")
        html.append("</tr>")
    return "".join(html) + "</table>"

def send_email(ce_df, pe_df, attachments) -> bool:
    sender, recipient = os.environ.get("SENDER_EMAIL"), os.environ.get("RECIPIENT_EMAIL")
    password = os.environ.get("SENDER_PASSWORD")
    if not sender or not recipient or not password: return False
    buy_view = prepare_option_email_view(ce_df, "long")
    short_view = prepare_option_email_view(pe_df, "short")
    html = f"<html><body>{colored_table_html(buy_view, OPTION_EMAIL_COLS, 'Buy Candidates')}{colored_table_html(short_view, OPTION_EMAIL_COLS, 'Short Candidates')}</body></html>"
    msg = MIMEMultipart()
    msg["From"], msg["To"], msg["Subject"] = sender, recipient, f"Intraday Alert - {datetime.now().strftime('%H:%M')}"
    msg.attach(MIMEText(html, "html", "utf-8"))
    # Add attachments... (same as previous)
...
def main():
    ...
    summary_df = pd.DataFrame([scan_symbol(s) for s in symbols if scan_symbol(s)])
    long_df, short_df = choose_top_candidates(summary_df)
    # The fix: Ensure BOTH build_option_candidates call both lists with proper filtering
    ce_df, _ = build_option_candidates(long_df, "long")
    pe_df, _ = build_option_candidates(short_df, "short")
    send_email(ce_df, pe_df, [])
