...
OPTION_EMAIL_COLS = [
    ...
    "OI", "Volume", "OBV", "OI+Volume+OBV Score", "EMAIL_RANK_SCORE", "Rank Delta",
    "Cumulative ADX", "5m_Signal", "15m_Signal", "30m_Signal", "60m_Signal",
...
def previous_trading_day_same_time_score(full_df: pd.DataFrame,
    ...
    if prev_day_data.empty:
        return np.nan
    same_time_rows = prev_day_data[(prev_day_data["timestamp"].dt.hour   == end_ts.hour) &
                                    (prev_day_data["timestamp"].dt.minute == end_ts.minute)]
    prev_end   = (pd.to_datetime(same_time_rows.iloc[-1]["timestamp"])
                  if not same_time_rows.empty else prev_day_data["timestamp"].iloc[-1])
    prev_start = prev_end - timedelta(minutes=window_minutes)
    prev_window = prev_day_data[(prev_day_data["timestamp"] >= prev_start) &
...
def summarize_intraday(intra_df: pd.DataFrame, reference_df: pd.DataFrame) -> Dict[str, object]:
    ...
    prev_close = (safe_float(reference_df["close"].iloc[-2])
                  if reference_df is not None and len(reference_df) >= 2 else np.nan)
...
def choose_top_candidates(summary_df: pd.DataFrame,
    ...
    long_df    = long_df.sort_values(["Rank Delta", "Cumulative ADX", "% Change"],
                                      ascending=[False, False, False]).head(top_n)
    short_df   = short_df.sort_values(["Rank Delta", "Cumulative ADX", "% Change"],
...
def rank_option_candidates(df: pd.DataFrame, side: str) -> pd.DataFrame:
    ...
    option_type = (out["Option Type"].astype(str).str.upper()
    ...
    if side == "long":
        ...
        out = out.sort_values(["EMAIL_RANK_SCORE", "Liq", "RD", "ADX", "PCT"],
...
def build_option_candidates(candidates_df: pd.DataFrame,
    ...
    if iter_rows and not final_out.empty:
        ...
        sort_cols  = [c for c in ["Underlying", "Option Type", "Strike", "Option Symbol", "iteration"]
                      if c in all_iters.columns]
        if sort_cols:
            all_iters = all_iters.sort_values(sort_cols).reset_index(drop=True)
        group_cols = [c for c in ["Underlying", "Option Type", "Strike", "Option Symbol"]
                      if c in all_iters.columns]
...
def prepare_option_email_view(df: pd.DataFrame, side: str, max_rows: int) -> pd.DataFrame:
    ...
    if timing_cols:
        ...
        mask = ~out[timing_cols].apply(
            lambda row: all(str(v).strip().upper() in neutral_like for v in row), axis=1)
        out = out[mask].copy()
    out = rank_option_candidates(out, side)
    final_cols = [c for c in OPTION_EMAIL_COLS if c in out.columns]
...
def send_obv_breakout_email(long_df: pd.DataFrame, short_df: pd.DataFrame,
...
# CHAIN-BASED SIGNAL SYSTEM  (additive â€” all original functions above unchanged)
# Entry : 5m CONFIRMED  +  30m CONFIRMED  +  T30 >= 10
# Exit  : 30m BROKEN    OR  T30 < 10
# Email : LONG (CE) + SHORT (PE) tables with sticky all-day rows
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
<style>
  ...
  h3{margin:14px 0 4px 0;padding:8px 12px;border-radius:4px;font-size:13px;}
  table{border-collapse:collapse;width:100%;margin-bottom:22px;}
  th{background:#1a1a2e;color:#e0e0e0;padding:6px 8px;font-size:11px;
     text-align:center;border:1px solid #444;}
  td{padding:5px 7px;border:1px solid #ddd;text-align:center;font-size:11px;}
...
def _compute_chain_status(signals: list):
    ...
    if t == 0:
        return "MIXED", 0, 0, 0
    ratio = b / t
    if ratio >= 0.65:   return "CONFIRMED", b, s, t
    elif ratio <= 0.35: return "BROKEN",    b, s, t
    return "MIXED", b, s, t


def _latest_chain_state(iter_df, block_minutes: int):
    if iter_df is None or iter_df.empty:
        return "MIXED", 0, 0, 0
    ipers   = max(1, block_minutes // SIGNAL_WINDOW_MINUTES)
    last_it = int(iter_df["iteration"].max())
    bn      = ((last_it - 1) // ipers) + 1
    b_start = (bn - 1) * ipers + 1
    b_end   = bn * ipers
    block   = iter_df[(iter_df["iteration"] >= b_start) & (iter_df["iteration"] <= b_end)]
    sigs    = block["window_signal"].tolist() if not block.empty else []
    return _compute_chain_status(sigs)


def scan_option_chain_signal(option_symbol: str, option_type: str,
                              strike: float, underlying: str):
    """Chain-signal scan for one option. Uses existing get_history + build_iteration_history."""
    sym      = option_symbol if option_symbol.startswith("NSE:") else f"NSE:{option_symbol}"
    ...
    if pd.isna(ltp) or ltp < MIN_OPTION_LTP:
        return None
    iter_df = build_iteration_history(intra_df)   # original function â€” unchanged
    if iter_df.empty:
        return None
    stat5,  _, _, t5  = _latest_chain_state(iter_df, 5)
    stat15, _, _, t15 = _latest_chain_state(iter_df, 15)
    stat30, _, _, t30 = _latest_chain_state(iter_df, 30)
    entry_ok = (stat5 == "CONFIRMED" and stat30 == "CONFIRMED" and t30 >= T30_MIN)
    ...
    return {
        ...
        "30m":          stat30,
        "T30":          t30,
        "Chain Signal": "ENTER" if entry_ok else ("EXIT" if exit_ok else "WAIT"),
        "Exit Signal":  "EXIT NOW" if exit_ok else "HOLD",
    }
...
def _update_sticky_rows(state, new_rows: list) -> list:
    existing  = state.get("rows", {})
    live_cols = ["LTP", "5m", "T5", "15m", "T15", "30m", "T30", "Chain Signal", "Exit Signal"]
    for row in new_rows:
...
def _chain_td(col, val):
    v   = str(val) if val is not None else ""
    cls = ""
    if col in ("5m", "15m", "30m"):
        cls = "confirmed" if v == "CONFIRMED" else ("broken" if v == "BROKEN" else "mixed")
    elif col == "Chain Signal":
        cls = "enter" if v == "ENTER" else ("exit_now" if v == "EXIT" else "wait")
    elif col == "Exit Signal":
        cls = "exit_now" if v == "EXIT NOW" else "hold"
    return f'<td class="{cls}">{v}</td>'


def _build_chain_table(rows: list, title: str, css_class: str) -> str:
    header = "".join(f"<th>{c}</th>" for c in CHAIN_DISPLAY_COLS)
    if rows:
        body = "".join(
            "<tr>" + "".join(_chain_td(c, r.get(c, "")) for c in CHAIN_DISPLAY_COLS) + "</tr>"
            for r in rows
        )
    else:
        body = (f'<tr><td colspan="{len(CHAIN_DISPLAY_COLS)}"'
                f' style="color:#999;padding:10px;">\u2014 No signals yet \u2014</td></tr>')
    return (f'<h3 class="{css_class}">{title}</h3>'
            f'<table><thead><tr>{header}</tr></thead>'
            f'<tbody>{body}</tbody></table>')


def _build_chain_email(long_rows: list, short_rows: list) -> str:
    ts        = datetime.now().strftime("%d %b %Y  %H:%M IST")
    long_tbl  = _build_chain_table(long_rows,  "\U0001f4c8 LONG CANDIDATES  (CE)", "long_h")
    short_tbl = _build_chain_table(short_rows, "\U0001f4c9 SHORT CANDIDATES (PE)", "short_h")
    return f"""<html><head>{CHAIN_EMAIL_STYLE}</head>
<body>
<p class="ts">Chain Signal Report \u2014 {ts}</p>
<p class="info">
  <b>Entry:</b> 5m CONFIRMED + 30m CONFIRMED + T30 &ge; {T30_MIN}
  &nbsp;|&nbsp;<b>Exit:</b> 30m BROKEN or T30 &lt; {T30_MIN}
  &nbsp;|&nbsp;<b>Sticky:</b> rows kept all day once ENTER fires
</p>
{long_tbl}
{short_tbl}
</body></html>"""
...
def run_chain_scan():
    """
    Chain-based signal scan â€” additive to main().
    Reuses all existing helpers: fetch_option_pairs, get_history,
    build_iteration_history, safe_float, send_single_email.
    """
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    state   = _load_chain_state()
    symbols = load_fno_symbols_from_sectors(SECTORS_DIR)[:TOP_N_UNDERLYINGS]
    all_scanned = []

    logger.info("Chain scan: %d symbols", len(symbols))
    for sym in symbols:
        try:
            pair_df = fetch_option_pairs(sym)
        except Exception as e:
            logger.warning("fetch_option_pairs %s: %s", sym, e)
            continue
        if pair_df.empty:
            continue
        # ATM volume gate
        atm_strike = pair_df["Strike"].iloc[0]
        any_pass   = False
        for req_type in ["CE", "PE"]:
            atm_sub = pair_df[(pair_df["Strike"] == atm_strike) &
                              (pair_df["Option Type"] == req_type)]
            vol = safe_float(atm_sub["Chain Volume"].iloc[0] if not atm_sub.empty else 0, 0)
            if vol >= MIN_ATM_CHAIN_VOLUME:
                any_pass = True
                break
        if not any_pass:
            logger.debug("SKIP %s: ATM chain volume too low", sym)
            continue
        for _, row in pair_df.iterrows():
            opt_sym  = str(row.get("Option Symbol", ""))
            opt_type = str(row.get("Option Type",  "")).upper()
            strike   = safe_float(row.get("Strike"), float("nan"))
            if not opt_sym or opt_type not in {"CE", "PE"} or pd.isna(strike):
                continue
            try:
                result = scan_option_chain_signal(opt_sym, opt_type, strike, sym)
            except Exception as e:
                logger.warning("scan_option_chain_signal %s: %s", opt_sym, e)
                result = None
            if result:
                all_scanned.append(result)
            time.sleep(PER_SYMBOL_SLEEP_SEC)

    all_rows   = _update_sticky_rows(state, all_scanned)
    long_rows  = sorted([r for r in all_rows if r.get("Option Type") == "CE"],
                        key=lambda r: r.get("Entry Time", "99:99"))
    short_rows = sorted([r for r in all_rows if r.get("Option Type") == "PE"],
                        key=lambda r: r.get("Entry Time", "99:99"))

    html     = _build_chain_email(long_rows, short_rows)
    ts_str   = datetime.now().strftime("%Y%m%d_%H%M")
    subject  = f"Chain Signal Report {datetime.now().strftime('%H:%M')}"
    long_csv  = os.path.join(OUTPUT_DIR, f"chain_long_{ts_str}.csv")
    short_csv = os.path.join(OUTPUT_DIR, f"chain_short_{ts_str}.csv")
    if long_rows:
        pd.DataFrame(long_rows).to_csv(long_csv, index=False)
    if short_rows:
        pd.DataFrame(short_rows).to_csv(short_csv, index=False)
    attachments = [f for f in [long_csv, short_csv] if os.path.exists(f)]
    send_single_email(subject, html, attachments)
    logger.info("Chain scan done \u2014 Long: %d  Short: %d", len(long_rows), len(short_rows))
...
def main() -> None:
    ...
    summary_df = summary_df.sort_values(
        ["Rank Delta", "% Change"], ascending=[False, False]).reset_index(drop=True)
    ...
    iteration_df = (pd.concat([long_iter_df, short_iter_df], ignore_index=True)
    ...
    # Chain signal scan + email (additive â€” runs after original emails)
    run_chain_scan()
