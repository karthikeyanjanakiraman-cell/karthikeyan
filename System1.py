...
# ==============================================================================
# 2. QUAD-DELTA VELOCITY ENGINE & NATIVE CUMULATIVE BASELINE
# ==============================================================================
def calculate_velocity_leaderboard(master_df, current_eval_time, window_mins=15):
    try:
        ...
        # Native 09:15 Market Open Cumulative Anchor
        ...
        # Strict Positive Score Threshold Lock (Eliminates Negative Score Leaks)
...
# ==============================================================================
# 3. STATE-BASED MEMORY ENGINE (Ironclad Gatekeeping & Lifecycle Bridges)
# ==============================================================================
def scan_institutional_tape(target_date_str):
    ...
    # ----------------------------------------------------------------------
    # LIVE EVALUATION LOOP (Multi-Basket Routing with Strict Memory Bridge)
    # ----------------------------------------------------------------------
            ...
            if not curr_anomalies.empty:
                for _, row in curr_anomalies.iterrows():
                    ...
                    if is_truly_fresh_today and sym not in all_fresh_intrusions:
                        ...
                        memory_bank[sym] = {
                            'state': 'ACTIVE', 'origin': price, 'date': target_date_str, 
                            'time': eval_time_current.strftime('%H:%M'), 'datetime': eval_time_current, 
                            'dir': direction, 'breach_time': None, 'breach_days': 0
                        }
                        ...
                        if st.get('date') != target_date_str or st.get('time') != eval_time_current.strftime('%H:%M'):
                            ...
                            # Prevent Same-Bar Reload Bug (must occur on a subsequent bar)
