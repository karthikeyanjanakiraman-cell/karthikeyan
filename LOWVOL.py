#!/usr/bin/env python3
"""
FO_FNO_FYERS_VOL_REL_EMAIL.py - Low-Volume (Liquidity Void) Support/Resistance Screener
"""
...
def scan_fno_universe(fyers):
    ...
    for sym in cfg.index_symbols:
        ...
        for lookback_days in range(base_lookback, max_lookback + 1, step):
            ...
            # MODIFICATION 1: Low Volume Focus
            # Filter zero-volume days first, then sort by volume ASCENDING
            valid_daily = daily[daily["volume"] > 0]
            work = valid_daily.sort_values(["volume", "high"], ascending=[True, False]).reset_index(drop=True)
...
def scan_options_universe(fyers, symbols):
    ...
    for sym in symbols:
        ...
        # MODIFICATION 2: Low Volume Focus for Options
        # Filter for days where volume actually exists to avoid dead zones
        valid_options_daily = daily[daily["volume"] > 0]
        if not valid_options_daily.empty:
            min_idx = valid_options_daily["volume"].idxmin()
        else:
            # Fallback to the tightest candle (smallest range) if volume fails
            min_idx = (daily["high"] - daily["low"]).idxmin()
...
def send_email(dashboard_df, long_df, short_df, ce_df, pe_df, csv_file):
    try:
        ...
        # MODIFICATION 3: Updated Email Subject to reflect Low-Volume
        msg["Subject"] = f"Index Low-Volume Dashboard - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        html = (
