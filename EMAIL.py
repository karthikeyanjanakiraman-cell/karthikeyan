...
""" def send_email_with_attachment(filename):
    ...
    Parameters:
    - filename: Path to the CSV file to attach
    ...
    try:
        ...
        msg['Subject'] = f"Strategy Results - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"

        # Email body
        body = f"""
Hello,

Please find attached the Asit Strategy v3.0 analysis results.
...
✅ GAP #1: RANK MAGNITUDE SCALING (15-tier continuous system)
✅ GAP #2: PYRAMID ENTRY SYSTEM (Scale-in tracking)
✅ GAP #3: IV RANK & IV PERCENTILE (Historical context)
✅ GAP #4: DAILY P&L TARGET (Walking away discipline)
✅ GAP #5: DELTA 0.30-0.60 RANGE (Optimal Greeks)
✅ GAP #6: PUT-CALL RATIO FILTER (Options flow)
✅ GAP #7: MULTI-INDICATOR EXIT (2-of-4 rule)
✅ GAP #8: PULLBACK ENTRY TIMING (Safer entries)
✅ GAP #9: AFTERNOON SWEET SPOT (1:30-2:00 PM IST)
✅ GAP #10: THETA DECAY MODELING (Exit timing)
✅ GAP #11: NEGATIVE SKEW PREFERENCE (IV optimization)
✅ GAP #12: LEVERAGE AVOIDANCE (Risk control)

This is an automated email from the Asit Strategy Trading System v3.0.

Best regards,
Asit Strategy Automated Analysis System
"""
        ...
        try:
            ...
            logger.error(f"[EMAIL] Failed to attach file: {str(e)}")
            print(f"[EMAIL] ❌ Failed to attach file: {str(e)}")
            return False
        ...
        try:
            ...
            print("[EMAIL] For Gmail: Use App Password from https://myaccount.google.com/apppasswords")
            print("[EMAIL] Check email_settings in config.ini\n")
            return False
        ...
        logger.warning("[EMAIL] ⚠️ No [email_settings] section found in config.ini - Skipping email")
        print("\n[EMAIL] ⚠️ Email settings not found in config.ini - Skipping email send")
        print("[EMAIL] Add [email_settings] section to config.ini to enable email reports\n")
        return False
    except configparser.NoOptionError as e:
        logger.warning(f"[EMAIL] ⚠️ Missing email configuration: {str(e)} - Skipping email")
        print(f"\n[EMAIL] ⚠️ Missing email configuration in config.ini: {str(e)}")
        print("[EMAIL] Check EMAIL_INTEGRATION_GUIDE.md for setup instructions\n")
        return False
    except Exception as e:
        logger.error(f"[EMAIL] ❌ Error reading email config: {str(e)}")
        print(f"[EMAIL] ❌ Error reading email config: {str(e)}")
        return False """
...
if __name__ == "__main__":
    ...
    email_sent = send_email_with_attachment(filename)
