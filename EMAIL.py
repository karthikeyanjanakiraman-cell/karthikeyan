...
def store_results_in_db(results_df):
    ...
    try:
        ...
        # CREATE TABLE with EXIT COLUMNS added
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS stock_signals (
        ...
        ON stock_signals(date, Symbol, run_time)
        ''')
        
        conn.commit()
        
        # INSERT DATA with exit columns
        run_time_now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ...
        for _, row in results_df.iterrows():
            try:
                ...
                (date, Symbol, run_time, RankScore_15Tier, DominantTrend, 
        ...
        conn.commit()
        logger.info(f"[DB] Stored {insert_count} signals for {today_str} at {run_time_now}")
        
        # Stats
        cursor.execute("SELECT COUNT(*) FROM stock_signals WHERE date = ?", (today_str,))
        total_today = cursor.fetchone()[0]
        logger.info(f"[DB] Total signals today: {total_today}")
...
def send_email_with_db_insights(csv_filename):
    ...
    try:
        ...
        # === BULLISH FRESH BREAKOUTS with EXIT columns ===
        query_bullish = '''
        WITH hist AS (
          SELECT
            date,
            Symbol,
            run_time,
            RankScore_15Tier AS latest_rank,
            DominantTrend,
            ShouldExit,
            ExitConfidence,
            ExitReason,
        ...
        breakouts AS (
          SELECT
            ...
            latest_rank,
            prev_max_rank,
            (latest_rank - prev_max_rank) AS breakout_size,
            DominantTrend,
            ShouldExit,
            ExitConfidence,
            ExitReason,
        ...
        SELECT
          Symbol,
          latest_rank AS RankScore,
          prev_max_rank AS PrevMax,
          breakout_size AS BreakoutSize,
          ShouldExit,
          ExitConfidence,
          ExitReason,
          ExitSignalsCount,
        ...
        WHERE
          ...
          AND breakout_size >= 0.5
        ORDER BY breakout_size DESC
        LIMIT 10
        '''
        
        cursor = conn.cursor()
        cursor.execute(query_bullish, (today_str,))
        rows = cursor.fetchall()
        
        for row in rows:
            top10_bullish.append({
                'Symbol': row[0],
                'RankScore': round(row[1], 2),
                'PrevMax': round(row[2], 2),
                'BreakoutSize': round(row[3], 2),
                'ShouldExit': 'YES' if row[4] == 1 else 'NO',
                'ExitConf%': round(row[5], 1),
                'ExitReason': row[6][:40],  # Truncate for table
                'ExitSignals': row[7],
                'LTP': round(row[8], 2),
                'Time': row[9]
            })
        
        # === BEARISH FRESH BREAKDOWNS with EXIT columns ===
        query_bearish = '''
        WITH hist AS (
          SELECT
            date,
        ...
        breakdowns AS (
          SELECT
            ...
            latest_rank,
            prev_min_rank,
            (prev_min_rank - latest_rank) AS breakdown_size,
            DominantTrend,
            ShouldExit,
            ExitConfidence,
            ExitReason,
        ...
        SELECT
          Symbol,
          latest_rank AS RankScore,
          prev_min_rank AS PrevMin,
          breakdown_size AS BreakdownSize,
          ShouldExit,
          ExitConfidence,
          ExitReason,
          ExitSignalsCount,
        ...
        WHERE
          ...
          AND breakdown_size >= 0.5
        ORDER BY breakdown_size DESC
        LIMIT 10
        '''
        
        cursor.execute(query_bearish, (today_str,))
        rows = cursor.fetchall()
        
        for row in rows:
            top10_bearish.append({
                'Symbol': row[0],
                'RankScore': round(row[1], 2),
                'PrevMin': round(row[2], 2),
                'BreakdownSize': round(row[3], 2),
                'ShouldExit': 'YES' if row[4] == 1 else 'NO',
                'ExitConf%': round(row[5], 1),
                'ExitReason': row[6][:40],
                'ExitSignals': row[7],
    ...
    # === BUILD EMAIL HTML with EXIT columns in tables ===
    
    # BULLISH table
    if top10_bullish:
        bullish_html = """
        <h3 style="color: #10b981; margin-top: 30px;">📈 Top 10 BULLISH Fresh Breakouts (Intraday New Highs)</h3>
        <p style="color: #6b7280;">Stocks breaking above their previous intraday RankScore maximum</p>
        <table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; width: 100%; font-size: 13px;">
          <thead style="background-color: #ecfdf5; font-weight: bold;">
            <tr>
              <th>Symbol</th>
              <th>RankScore</th>
              <th>PrevMax</th>
          ...
          <tbody>
        """
        for item in top10_bullish:
            exit_color = '#ef4444' if item['ShouldExit'] == 'YES' else '#10b981'
            bullish_html += f"""
            <tr>
              <td style="font-weight: bold;">{item['Symbol']}</td>
              <td>{item['RankScore']}</td>
              <td style="color: #6b7280;">{item['PrevMax']}</td>
              <td style="color: #10b981; font-weight: bold;">+{item['BreakoutSize']}</td>
              <td>₹{item['LTP']}</td>
              <td style="color: {exit_color}; font-weight: bold;">{item['ShouldExit']}</td>
              <td>{item['ExitConf%']}%</td>
              <td style="font-size: 11px;">{item['ExitReason']}</td>
              <td>{item['ExitSignals']}</td>
              <td style="font-size: 11px; color: #6b7280;">{item['Time']}</td>
            </tr>
            """
        bullish_html += "</tbody></table>"
    else:
        bullish_html = """
        <h3 style="color: #10b981; margin-top: 30px;">📈 Top 10 BULLISH Fresh Breakouts</h3>
        <p style="color: #9ca3af; font-style: italic;">No fresh breakouts (need 2+ runs for comparison)</p>
        """
    
    # BEARISH table
    if top10_bearish:
        bearish_html = """
        <h3 style="color: #ef4444; margin-top: 30px;">📉 Top 10 BEARISH Fresh Breakdowns (Intraday New Lows)</h3>
        <p style="color: #6b7280;">Stocks breaking below their previous intraday RankScore minimum</p>
        <table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; width: 100%; font-size: 13px;">
          <thead style="background-color: #fee2e2; font-weight: bold;">
            <tr>
              <th>Symbol</th>
              <th>RankScore</th>
              <th>PrevMin</th>
          ...
          <tbody>
        """
        for item in top10_bearish:
            exit_color = '#ef4444' if item['ShouldExit'] == 'YES' else '#10b981'
            bearish_html += f"""
            <tr>
              <td style="font-weight: bold;">{item['Symbol']}</td>
              <td>{item['RankScore']}</td>
              <td style="color: #6b7280;">{item['PrevMin']}</td>
              <td style="color: #ef4444; font-weight: bold;">-{item['BreakdownSize']}</td>
              <td>₹{item['LTP']}</td>
              <td style="color: {exit_color}; font-weight: bold;">{item['ShouldExit']}</td>
              <td>{item['ExitConf%']}%</td>
              <td style="font-size: 11px;">{item['ExitReason']}</td>
            ...
            """
        bearish_html += "</tbody></table>"
    else:
        bearish_html = """
        <h3 style="color: #ef4444; margin-top: 30px;">📉 Top 10 BEARISH Fresh Breakdowns</h3>
        <p style="color: #9ca3af; font-style: italic;">No fresh breakdowns (need 2+ runs)</p>
        """
    
    # Complete email body
    body_html = f"""
    <html>
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #1f2937; max-width: 900px; margin: 0 auto; padding: 20px;">
      <h2 style="color: #1f2937; border-bottom: 3px solid #3b82f6; padding-bottom: 10px;">
        🎯 ASIT Intraday Greeks v3.0 Report
      </h2>
      
      <p style="color: #4b5563;">
        <strong>Generated:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S IST')}<br>
        <strong>File:</strong> {csv_filename}<br>
        <strong>System:</strong> v3.0 with ALL 12 GAPS FIXED + Fresh Breakout/Breakdown Detection
      </p>
      
      <div style="background-color: #f3f4f6; padding: 15px; border-radius: 8px; margin: 20px 0;">
        <h3 style="margin-top: 0; color: #3b82f6;">📊 Intraday Fresh Momentum Signals</h3>
        <p style="margin-bottom: 5px; color: #6b7280;">
          Real-time detection of stocks making <strong>new intraday RankScore extremes</strong> vs their own previous runs today.
        </p>
        <ul style="margin: 10px 0; padding-left: 20px; color: #4b5563;">
          <li><strong>Breakout:</strong> Latest RankScore exceeds all prior intraday maximums (momentum accelerating)</li>
          <li><strong>Breakdown:</strong> Latest RankScore falls below all prior intraday minimums (weakness intensifying)</li>
          <li><strong>Exit Signals:</strong> Multi-indicator exit confirmation (2-of-4 rule: Supertrend, MACD, ADX, Price)</li>
        </ul>
        <p style="margin-top: 10px; padding: 10px; background-color: #fef3c7; border-left: 4px solid #f59e0b; font-size: 13px;">
          ⚠️ <strong>Note:</strong> Requires 2+ runs today for comparison. First run shows empty tables.
        </p>
      </div>
      ...
      <div style="margin-top: 40px; padding-top: 20px; border-top: 2px solid #e5e7eb;">
        <p style="color: #6b7280; font-size: 13px;">
          Complete analysis with all stocks attached as CSV.<br>
          This is an automated email from ASIT Strategy v3.0 Trading System.
        </p>
        <p style="color: #9ca3af; font-size: 12px; margin-top: 15px;">
          <strong>Exit Legend:</strong><br>
          • <span style="color: #ef4444; font-weight: bold;">YES</span> = 2+ exit indicators confirming (consider closing position)<br>
          • <span style="color: #10b981; font-weight: bold;">NO</span> = Hold (exit signals &lt; 2)<br>
          • Exit Conf% = Confidence level of exit signal (70%+ = strong)<br>
          • Exit Signals = Count of active exit indicators (Supertrend, MACD, ADX, Price, Theta)
        </p>
      </div>
    </body>
    </html>
    ...
    try:
        ...
        logger.error(f"❌ Email build error: {e}")
        return False
