...
def sendemail_rank_watchlist(csv_filename: str) -> bool:
    ...
    Watchlist rules:
      - First run of the day: take TOP 10 bullish / TOP 10 bearish by RankScore15Tier.
      - On each subsequent run:
          • Compute TOP 10 bullish / bearish for that run.
          • Any symbol that appears in TOP 10 (and was not already in the watchlist)
    ...
    Prev / Diff:
      - Prev Score = previous intraday MAX (bullish) / MIN (bearish) RankScore15Tier
        for that symbol, based on all rows earlier in the day.
      - Diff = Latest Score − Prev Score (signed; can be +, −, or 0).
      - For symbols with only one row (no previous extreme), Prev = Latest, Diff = 0.
      - For APPENDED symbols on the first time they appear in DB, Prev = N/A, Diff = N/A.
    """
    from datetime import date as _date
    ...
    # --- Load today's rows from stocksignals ---
    try:
        conn = sqlite3.connect(DB_PATH)
    ...
    # --- Intraday previous max/min and diff calculation ---
    ...
    # Tag each symbol as FIRST_RUN or APPENDED based on earliest time they entered TOP10
    ...
    def _build_display(df_side: _pd.DataFrame, side: str) -> _pd.DataFrame:
        ...
        # Sort for nice display: bulls by Latest desc, bears by Latest asc
        try:
...
def init_daily_db():
    """
    Initialize DB and clear old data, keep only today.
    This is the single place that owns the stocksignals schema.
    """
    from datetime import date as _date
    today_str = _date.today().strftime("%Y-%m-%d")
    ...
    rowcount = conn.execute(
        "SELECT COUNT(*) FROM stocksignals WHERE date = ?",
        (today_str,)
    ).fetchone()[0]
...
def store_results_in_db(df: pd.DataFrame):
    """
    Store current run results in DB, appending today's rows.
    No hard‑coded dates; uses today's date and current time.
    """
    if df is None or df.empty:
    ...
    from datetime import date as _date, datetime as _dt
    today_str = _date.today().strftime("%Y-%m-%d")
    runtime_str = _dt.now().strftime("%H:%M:%S")

    conn = sqlite3.connect(DB_PATH)

    df_store = df.copy()

    # normalize / add columns for DB
    df_store["date"] = today_str
    df_store["runtime"] = runtime_str
    df_store["Symbol"] = df_store.get("Symbol", df_store.get("symbol", ""))
    df_store["RankScore15Tier"] = df_store.get(
        "RankScore15Tier",
        df_store.get("RankScore_15Tier", df_store.get("rankscore", 0))
    )
    df_store["BullMultiTFScore"] = df_store.get(
        "Bull_MultiTF_Score", df_store.get("BullMultiTFScore", 0)
    )
    df_store["BearMultiTFScore"] = df_store.get(
        "Bear_MultiTF_Score", df_store.get("BearMultiTFScore", 0)
    )
    df_store["DominantTrend"] = df_store.get(
        "DominantTrend", df_store.get("trend", "NEUTRAL")
    )
    df_store["TrendStrength"] = df_store.get("TrendStrength", 0)
    df_store["PositionSizeMultiplier"] = df_store.get(
        "PositionSizeMultiplier", df_store.get("pos_multiplier", 1.0)
    )
    df_store["EntryConfidence"] = df_store.get("EntryConfidence", 0)
    df_store["LTP"] = df_store.get(
        "LTP", df_store.get("ltp", df_store.get("Close", 0))
    )
    df_store["CanTradeToday"] = df_store.get("CanTradeToday", True)

    cols = [
        "date", "runtime", "Symbol", "RankScore15Tier",
        "BullMultiTFScore", "BearMultiTFScore",
        "DominantTrend", "TrendStrength",
        "PositionSizeMultiplier", "EntryConfidence",
        "LTP", "CanTradeToday",
    ]
...
if __name__ == "__main__":
    ...
    print(" WITH RANK SCALING + PYRAMID ENTRY + IV RANK + DAILY P&L + DELTA RANGE + PCR + "
          "MULTI-EXIT + PULLBACK + AFTERNOON + THETA + SKEW + LEVERAGE")
    print("=" * 100 + "\n")
    ...
    # STEP 3: Store in DB (single, consistent path)
