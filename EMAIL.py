...
"""
═══════════════════════════════════════════════════════════════════════════════════════════════════
ASIT BARAN PATI STRATEGY - PRODUCTION v12.4 - KINETIC VELOCITY EDITION
PURE VOLUME FLOW + TIME-TO-VOLUME ANCHOR + EXPONENTIAL BACKOFF API PROTECT
═══════════════════════════════════════════════════════════════════════════════════════════════════
"""
...
HISTORY_CSV = f"asit_kinetic_history_{TODAY_STR}.csv"
...
# ═══════════════════════════════════════════════════════════════════════════════════════════════════
# KINETIC VOLUME DATA PROCESSING ENGINE
# ═══════════════════════════════════════════════════════════════════════════════════════════════════
def process_kinetic_indicators(df, tf_name, is_daily=False):
    ...
    # 2. Synthetic Volume OHLC & Directional Flow
    ...
    # 3. Average True Volume Range (ATVR)
    df['v_change_abs'] = df['volume'].diff().abs()
    df['atvr'] = df['v_change_abs'].rolling(window=14).mean().fillna(1)
    
    # 4. Kinetic Velocity Calculations (Time to achieve 10,000 volume)
    if tf_name == '15min': sec = 900
    elif tf_name == '1hour': sec = 3600
    else: sec = 22500  # Total operational seconds in NSE trading day
    ...
    # Speed Efficiency Ratio calculation
    df['speed_efficiency'] = df['peak_time_10k'] / df['cur_time_10k']
    
    # 5. Volume Fractals (Institutional Activity Nodes)
    ...
    # 6. Historical Volume Volatility (HVV)
    if is_daily:
...
def calc_drift_bull(df, is_daily):
    ...
    score = 0
    latest = df.iloc[-1]
    speed_eff = latest.get('speed_efficiency', 0)
    
    if latest['close'] > latest.get('vwap', 0): score += 0.20
    if latest.get('obv', 0) > latest.get('obv_ema10', float('inf')): score += 0.20
    
    # Kinetic matching parameters
    if speed_eff >= 0.80: score += 0.30       
    elif speed_eff >= 0.50: score += 0.15     
    
    if latest.get('v_roc', 0) > 0: score += 0.15
    if latest.get('v_adx', 0) > 25: score += 0.15
    return min(score, 1.0)
...
def scan_symbol(symbol):
    ...
    for tf_name, cfg in TIMEFRAMES.items():
        ...
        df_ind = process_kinetic_indicators(df, tf_name, is_daily)
...
# ═══════════════════════════════════════════════════════════════════════════════════════════════════
# MOMENTUM DIVERGENCE CLASSIFIER & EMAIL
# ═══════════════════════════════════════════════════════════════════════════════════════════════════
    ...
    for idx, row in df.reset_index(drop=True).iterrows():
        ...
        html += f"""
            <tr style="background-color: {bg_color}; border-bottom: 1px solid #333333;">
                ...
                <td style="padding: 10px 8px; font-weight: 500; color: #4caf50;">{row['Peak_10k']}</td>
                <td style="padding: 10px 8px; font-weight: 500; color: {cur_color};">{row['Cur_10k']}</td>
...
def send_email_report(df):
    ...
    # Sort validation matrix by current velocity tracking
    ...
    msg["Subject"] = f"Elite Kinetic Matrix (Volume Velocity) - {datetime.now().strftime('%H:%M')}"
    ...
    <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #121212; color: #e0e0e0; padding: 20px;">
      ...
      <div style="text-align: center; margin-bottom: 30px;">
          <h2 style="color: #ffffff; margin-bottom: 5px;">Kinetic Velocity Institutional Matrix (v12.4)</h2>
          <p style="color: #aaaaaa; margin-top: 0;"><b>Kinetic Anchor Engine Active:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
      </div>
      ...
      <div style="margin-top: 40px; padding: 15px; background-color: #1e1e1e; border-radius: 8px; border-left: 4px solid #d4af37;">
          <h4 style="color: #ffffff; margin-top: 0;">Kinetic System Documentation:</h4>
          <ul style="color: #cccccc; line-height: 1.6;">
            <li><b>Peak 10k Spd:</b> The absolute fastest time it took for an institution to clear 10,000 shares today.</li>
            <li><b>Cur 10k Spd:</b> The liquidity velocity on the most recent tracking execution bar.</li>
            <li><b>Drift Score:</b> Measures speed efficiency. If execution speed drops below 50% of the session high, the stock drops off the tracking ledger.</li>
          </ul>
      </div>
    </body>
    </html>
    ...
    try:
        ...
        server.quit()
        logger.info(f"[EMAIL] Kinetic matrix successfully dispatched.")
    except Exception as e:
...
def main():
    print("=" * 80)
    print("[LAUNCH] ASIT v12.4 - KINETIC VELOCITY EDITION")
    print("=" * 80)
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        for idx, future in enumerate(as_completed(futures), 1):
            try:
                ...
                if idx % 20 == 0: logger.info(f"[SCAN] Processing Kinetic Matrix: {idx}/{len(symbols)} complete...")
            except Exception as e:
                pass
