...
OPTION_EMAIL_COLS = [
    ...
    "OI+Volume+OBV Score", "EMAIL_RANK_SCORE", "Rank Delta", "Cumulative ADX", "5m_Signal", "15m_Signal",
    "30m_Signal", "60m_Signal", "Bull_Signal", "Bear_Signal", "Overall_Signal", "Price_Lead_Status", "IVP",
...
def compute_chain_status(signals: List[str]) -> Tuple[str, int, int, int]:
    buy_count = sum(1 for s in signals if str(s).startswith("Buy"))
    sell_count = sum(1 for s in signals if str(s).startswith("Sell"))
    total = buy_count + sell_count
    if total == 0:
        return "MIXED", 0, 0, 0
    ratio = buy_count / total
    if ratio >= 0.65:
        return "CONFIRMED", buy_count, sell_count, total
    if ratio <= 0.35:
        return "BROKEN", buy_count, sell_count, total
    return "MIXED", buy_count, sell_count, total


def latest_block_chain(iter_df: pd.DataFrame, block_minutes: int) -> Tuple[str, int, int, int]:
    ...
    last_iteration = int(iter_df["iteration"].max())
    block_num = (last_iteration - 1) // iters_per_block + 1
    block_start = (block_num - 1) * iters_per_block + 1
    block_end = block_num * iters_per_block
    block_rows = iter_df[(iter_df["iteration"] >= block_start) & (iter_df["iteration"] <= block_end)]
    signals = block_rows["windowsignal"].tolist() if not block_rows.empty and "windowsignal" in block_rows.columns else []
    return compute_chain_status(signals)


def scan_option_chain_signals(option_symbol: str, option_type: str, strike: float, underlying: str, iter_df: pd.DataFrame) -> Dict:
    ...
    chain_signal = "ENTER" if entry_signal else "EXIT" if exit_signal else "WAIT"
...
def update_sticky_rows(state: Dict, new_rows: List[Dict]) -> List[Dict]:
    ...
    for row in new_rows:
        ...
        if key not in existing:
            row = dict(row)
            if row.get("Chain Signal") == "ENTER" and "Entry Time" not in row:
...
def compact_table_html(df: pd.DataFrame, title: str, max_rows: int) -> str:
    ...
    cols = [c for c in ["Underlying", "Option Type", "Strike", "LTP", "% Change", "OI", "Volume", "OBV", "Liq Score", "Rank", "Rank Delta", "Cumulative ADX", "5m_Signal", "15m_Signal", "Overall_Signal", "IVP", "Time"] if c in df.columns]
    df = df[cols].head(max_rows).copy()
...
def build_chain_email_html(long_rows: List[Dict], short_rows: List[Dict]) -> str:
    ...
    return f"<html><body><p>Chain Signal Report - {ts}</p>{compact_table_html(pd.DataFrame(long_rows), 'LONG CANDIDATES CE', EMAIL_MAX_ROWS_LONG)}{compact_table_html(pd.DataFrame(short_rows), 'SHORT CANDIDATES PE', EMAIL_MAX_ROWS_SHORT)}</body></html>"
...
def build_cepebuy_email_html(ce_rows: List[Dict], pe_rows: List[Dict]) -> str:
    ...
    def table(rows, title):
        ...
        header = ''.join(f"<th>{c}</th>" for c in cols)
        body = ''.join('<tr>' + ''.join(f'<td>{row[c]}</td>' for c in cols) + '</tr>' for _, row in df[cols].iterrows())
        return f"<h3>{title}</h3><table border='1' cellspacing='0' cellpadding='4'><thead><tr>{header}</tr></thead><tbody>{body}</tbody></table>"
    return f"<html><body><p>CE PE Momentum Buy Report - {ts}</p><p>Rule: 8 of last 11 same-direction windows.</p>{table(ce_rows, 'CE BUY MOMENTUM SIGNALS')}{table(pe_rows, 'PE BUY MOMENTUM SIGNALS')}</body></html>"
...
def main():
    ...
    if not long_iter_df.empty:
        for keys, grp in long_iter_df.groupby([c for c in ["Underlying", "Option Type", "Strike", "Option Symbol"] if c in long_iter_df.columns]):
            if not isinstance(keys, tuple):
                keys = (keys,)
            cols = [c for c in ["Underlying", "Option Type", "Strike", "Option Symbol"] if c in long_iter_df.columns]
            keymap = dict(zip(cols, keys))
            sig = scan_option_chain_signals(keymap.get("Option Symbol", ""), keymap.get("Option Type", ""), safe_float(keymap.get("Strike"), np.nan), keymap.get("Underlying", ""), grp)
            if sig:
                long_merged.append(sig)
    if not short_iter_df.empty:
        for keys, grp in short_iter_df.groupby([c for c in ["Underlying", "Option Type", "Strike", "Option Symbol"] if c in short_iter_df.columns]):
            if not isinstance(keys, tuple):
                keys = (keys,)
            ...
            sig = scan_option_chain_signals(keymap.get("Option Symbol", ""), keymap.get("Option Type", ""), safe_float(keymap.get("Strike"), np.nan), keymap.get("Underlying", ""), grp)
            if sig:
                short_merged.append(sig)
    state = load_daily_state()
    state["rows"] = update_sticky_rows(state, long_merged + short_merged)
    save_daily_state(state)
