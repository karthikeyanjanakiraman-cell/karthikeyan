...
- UNIFIED (AND Condition): Blends market-wide Vol_Ratio leaders that ALSO clear the Volat Exp hurdle.
═══════════════════════════════════════════════════════════════════════════════════════════════════
"""
...
def send_html_email(df_matrix, target_dt):
    ...
    def build_rows(df):
        if df.empty:
            return "<tr><td colspan='7' style='text-align:center; color:#757575;'>No structures cleared targets.</td></tr>"
    ...
    <html lang="en">
      ...
      <body>
        ...
        <p style="font-size: 12px; color: #777; text-align: center;">
            TMV Engine v25.0 • Anchored Watchlist Size: {WATCHLIST_SIZE} • Speed Hurdle Dial: {int(SPEED_THRESHOLD_RATIO*100)}% • Volat Exp Hurdle: >{VOLATILITY_EXP_THRESHOLD}x (Leaders Appended)
        </p>
      </body>
    </html>
    """
...
def main():
    ...
    # -----------------------------------------------------------------
    # PHASE 2: CONTINUOUS INTERVAL LOOP PROCESSING VIA WATCHLIST & SCANNER
    # -----------------------------------------------------------------
    current_dt = start_dt
    ...
    while current_dt <= end_dt:
        ...
        # Process the entire F&O list in memory to pull the cross-market volume leaders
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = {
                executor.submit(process_intraday_matrix, row['Symbol'], row['Pre_Market_Ratio'], row['Full_DF'], current_dt): row['Symbol'] 
                for _, row in pre_market_df.iterrows()
            }
            for future in as_completed(futures):
                res = future.result()
                if res: all_intraday_results.append(res)
                
        all_matrix = pd.DataFrame(all_intraday_results)
        
        if not all_matrix.empty:
            
            # 1. APPLY UNIVERSAL 'AND' CONDITION: Every stock (Anchored or Market-wide) MUST pass Volat_Exp
            passed_volat_matrix = all_matrix[all_matrix['Volat_Ratio'] > VOLATILITY_EXP_THRESHOLD]
            
            # 2. Extract the anchored watchlist members that passed the hurdle
            anchored_clean_symbols = [s.replace('NSE:', '').replace('-EQ', '') for s in anchored_watchlist['Symbol']]
            filtered_anchored = passed_volat_matrix[passed_volat_matrix['Symbol'].isin(anchored_clean_symbols)]
            
            # 3. Extract the absolute top 10 highest Volume Ratio stocks across the entire market (that ALSO passed the hurdle)
            top_vol_market = passed_volat_matrix.sort_values('Vol_Ratio', ascending=False).head(10)
            
            # 4. Concatenate both structures together into one single dataset and drop any duplicate symbols
            unified_matrix = pd.concat([filtered_anchored, top_vol_market]).drop_duplicates(subset=['Symbol'])
            
            # 5. Sort the single unified table cleanly by your institutional preference dials
            PRIORITY_SORT = ['Volat_Ratio', 'Vol_Ratio', 'Pre_Market_Ratio']
            sorted_unified_matrix = unified_matrix.sort_values(PRIORITY_SORT, ascending=[False, False, False])
            
            # Dispatch via the single master table structure
            if not sorted_unified_matrix.empty:
                send_html_email(sorted_unified_matrix, current_dt)
            else:
                logger.warning(f"No structures cleared the > {VOLATILITY_EXP_THRESHOLD}x hurdle at {current_dt.strftime('%I:%M %p')}. Email skipped.")
        else:
