...
def _extract_underlying(sym):
    """
    Strips an NSE option contract down to its underlying symbol, e.g.
    'NSE:360ONE26SEP1140PE' -> '360ONE'. Cash-equity symbols (no option
    suffix) are returned unchanged. Used only for the basket summary counts.
    """
    s = sym.replace("NSE:", "")
...
def display_final_results(tape_exec, memory_bank, target_dt, target_date_str):
    ...
    for st in active_runners: active_counts[_extract_underlying(st["sym"])] += 1
    for st in closed_trades: closed_counts[_extract_underlying(st["sym"])] += 1
    ...
    print(f"{COLOR_CYAN}------------------------------------------------------------------------------------------------{COLOR_RESET}")
    print(f"{COLOR_BOLD}📊 SUMMARY — Trade Count by Underlying{COLOR_RESET}")
    if all_underlyings:
...
class LiveWebsocketEngine:
    ...
    def run_event_loop(self):
        ...
        # Now it stops cleanly once the market closes, prints one final result
        # snapshot, and returns so the script exits normally.
        market_close_t = datetime.strptime("15:30:00", "%H:%M:%S").time()
        while True:
            now = datetime.now()
            if now.time() >= market_close_t:
                print(f"\n{COLOR_YELLOW}🔔 Market closed (15:30). Stopping live engine.{COLOR_RESET}")
                break
            time.sleep(1)
            now = datetime.now()
