...
def compute_row(symbol, master_1m):
    ...
    for mult in HA_ATR_MULTIPLIERS:
        ...
        if alignment == "BULL":
            ...
            row['Score'] -= 1
        if mult == 1:
            # alignment already reflects the HA-trend check AND the Warzone veto --
            # this is the one true "did the anchor actually qualify" answer.
...
def run_screener(mode=TRADING_MODE, days=BACKTRACE_DAYS, min_blocks=MIN_PERFECT_BLOCKS):
    ...
    # Anchor1XBull/Anchor1XBear already incorporate the HA-trend confirmation
    # AND the Warzone veto -- gate on them directly so a block that got killed
    # as chop can't sneak back in by re-checking only the raw RSI/MACD/ADX columns.
    bulls = [r for r in dashboard_data if r['PerfectBullBlocks'] >= min_blocks and r.get('Anchor1XBull')]
    bears = [r for r in dashboard_data if r['PerfectBearBlocks'] >= min_blocks and r.get('Anchor1XBear')]
